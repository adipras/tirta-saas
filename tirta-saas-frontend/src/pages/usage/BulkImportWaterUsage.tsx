...
import { ArrowLeftIcon, ArrowUpTrayIcon, CheckCircleIcon, XCircleIcon, DocumentArrowDownIcon, InformationCircleIcon } from '@heroicons/react/24/outline';
  ...
  const handleDownloadExcelTemplate = async () => {
    ...
    try {
      ...
      const rows = customers
        ...
        .map((c) => ({ meter_number: c.meter_number, customer_name: c.name, meter_end: '', notes: '' }));

      const aoa = [
        USAGE_TEMPLATE_HEADERS,
        ...rows.map((r) => USAGE_TEMPLATE_HEADERS.map((h) => (r as Record<string, string | number>)[h] ?? '')),
      ];
      const ws = XLSX.utils.aoa_to_sheet(aoa);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Pemakaian');
      XLSX.writeFile(wb, 'template_pemakaian_air.xlsx');
      toast.success(`Template berhasil diunduh — ${rows.length} pelanggan`);
    } catch {
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-md bg-amber-50 border border-amber-200 p-4 flex gap-3">
        ...
        <div className="text-sm text-amber-800 space-y-1">
          <p className="font-semibold">Pertama kali import? Pastikan Initial Reading sudah diset terlebih dahulu.</p>
          <p>
