...
const (
	DefaultInvoiceGenerationDay = 5
	DefaultInvoiceDueDay        = 25
	DefaultInvoiceDueDays       = 20
...
type TenantSettings struct {
	...
	InvoiceGenerationDay int    `gorm:"default:5" json:"invoice_generation_day"`
	InvoiceDueDay        int    `gorm:"default:25" json:"invoice_due_day"`
	...
	InvoiceDueDays       int    `gorm:"default:20" json:"invoice_due_days"`
...
func (ts *TenantSettings) ApplyBillingDefaults() {
	if ts.InvoiceGenerationDay <= 0 {
		ts.InvoiceGenerationDay = DefaultInvoiceGenerationDay
	}
	if ts.InvoiceDueDay <= 0 {
		ts.InvoiceDueDay = DefaultInvoiceDueDay
	}
	if ts.InvoiceDueDays <= 0 {
		ts.InvoiceDueDays = DefaultInvoiceDueDays
	}
	if ts.GracePeriodDays < 0 {
		ts.GracePeriodDays = DefaultGracePeriodDays
	}
}

// BeforeCreate sets default values for JSON fields
func (ts *TenantSettings) BeforeCreate(tx *gorm.DB) error {
	if err := ts.BaseModel.BeforeCreate(tx); err != nil {
		return err
	}

	ts.ApplyBillingDefaults()
